...
describe('Starting tests for Mahak', function() {
  ...
  it('website title should be Mahak', () => {
    page.navigateTo('/');
    return browser.getTitle().then((result)=>{
      expect(result).toBe('Mahak');
    })
  });
  ...
  it('navbar-brand should be Mahak',() => {
    element(by.css('.navbar-brand')).getWebElement()
    .then((webElement) => {
      return webElement.getText();
    })
    .then((txt) => {
      expect(txt).toBe('Mahak');
    });
  });
